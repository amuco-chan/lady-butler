...
const generateSecureSyncToken = () => {
  const bytes = new Uint8Array(24)
  crypto.getRandomValues(bytes)
  const hex = Array.from(bytes, byte => byte.toString(16).padStart(2, '0')).join('')
  return `lb-${hex.match(/.{1,4}/g)!.join('-')}`
}
...
type CalendarConnectStatus = 'idle' | 'checking' | 'connected' | 'disconnected' | 'unconfigured' | 'invalid' | 'syncing' | 'error'
...
const isEmailInboxItem = (item: GptInboxItem) => {
  const text = `${item.memo} ${item.sourceText} ${(item.ambiguities ?? []).join(' ')}`
  return /Gmailより|メールより|Gmail由来|メール由来/.test(text)
}
...
export default function App() {
  ...
  const saveTask = (task: Task) => {
    ...
    const cleanTask = { ...task, taskType: kind, lastCompletedDate: kind === 'daily' ? task.status === '完了' ? task.lastCompletedDate || localDate() : '' : '', actualMinutes: taskActualMinutes(task) }
  ...
  const saveEvent = (event: CalendarEvent) => {
    const cleanEvent = event.source === 'google' ? event : { ...event, sourceEventId: undefined }
  ...
  const importCalendarEvents = (incoming: CalendarEvent[]) => setEvents(prev => {
    ...
    const sourceIds = new Set(prev
      .map(event => event.source === 'google' && event.sourceEventId ? `google|${event.calendarId || ''}|${event.sourceEventId}` : '')
      .filter(Boolean))
    const fresh = incoming.filter(event => {
      const sourceKey = event.source === 'google' && event.sourceEventId ? `google|${event.calendarId || ''}|${event.sourceEventId}` : ''
      return sourceKey ? !sourceIds.has(sourceKey) : !signatures.has(`${event.title}|${event.startAt}|${event.endAt}`)
    })
  ...
  const dismissEmailInboxItems = () => {
    const dismissed = gptInbox.filter(isEmailInboxItem).length
    if (!dismissed) return
    setImportNotice(`メール由来の候補${dismissed}件を見送りました。`)
    setGptInbox(prev => prev.filter(item => !isEmailInboxItem(item)))
  }
  ...
  useEffect(() => {
    ...
    if (params.has('calendar')) setPage('calendar')
    if (params.has('gmail')) setPage('tasks')
  }, [])
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="page-wrap">
        ...
        {page === 'home' && <HomePage name={settings.name.trim() || 'レディ'} settings={settings} tasks={tasks} taskWorkLogs={taskWorkLogs} events={events} moodLogs={moodLogs} gptInbox={gptInbox} importNotice={importNotice} go={changePage} acceptInboxItem={acceptInboxItem} reviewInboxItem={reviewInboxItem} dismissInboxItem={dismissInboxItem} dismissEmailInboxItems={dismissEmailInboxItems}/>}
        {page === 'tasks' && <TasksPage tasks={tasks} taskWorkLogs={taskWorkLogs} syncToken={syncToken} gptInbox={gptInbox} importNotice={importNotice} edit={task => { setReviewingInbox(null); setEditing(task) }} remove={id => setTasks(p => p.filter(t => t.id !== id))} complete={complete} logTime={logTaskTime} logFocus={logFocusTime} acceptInboxItem={acceptInboxItem} reviewInboxItem={reviewInboxItem} dismissInboxItem={dismissInboxItem} dismissEmailInboxItems={dismissEmailInboxItems} importEmailCandidates={importEmailCandidates}/>}
...
function InboxReviewCard({ gptInbox, importNotice, acceptInboxItem, reviewInboxItem, dismissInboxItem, dismissEmailInboxItems, eyebrow = 'GPT AUTO SYNC' }: { gptInbox: GptInboxItem[]; importNotice: string; acceptInboxItem: (item: GptInboxItem) => void; reviewInboxItem: (item: GptInboxItem) => void; dismissInboxItem: (id: string) => void; dismissEmailInboxItems?: () => void; eyebrow?: string }) {
  if (!importNotice && gptInbox.length === 0) return null
  const emailCount = gptInbox.filter(isEmailInboxItem).length
  return <section className="card gpt-inbox-card"><div className="section-title"><div><span>{eyebrow}</span><h2>{gptInbox.length ? '少しだけ確認してください' : '自動で反映しました'}</h2></div><div className="inbox-title-actions"><small>{gptInbox.length ? `要確認 ${gptInbox.length}件` : '操作不要'}</small>{emailCount > 0 && dismissEmailInboxItems && <button type="button" className="inbox-dismiss-email" onClick={dismissEmailInboxItems}>メール候補を全て見送る</button>}</div></div>{importNotice && <p className="inbox-notice">{importNotice}</p>}{gptInbox.length ? <div className="inbox-list">{gptInbox.map(item => {
...
function HomePage({ name, settings, tasks, taskWorkLogs, events, moodLogs, gptInbox, importNotice, go, acceptInboxItem, reviewInboxItem, dismissInboxItem, dismissEmailInboxItems }: { name: string; settings: Settings; tasks: Task[]; taskWorkLogs: TaskWorkLog[]; events: CalendarEvent[]; moodLogs: MoodLog[]; gptInbox: GptInboxItem[]; importNotice: string; go: (p: Page) => void; acceptInboxItem: (item: GptInboxItem) => void; reviewInboxItem: (item: GptInboxItem) => void; dismissInboxItem: (id: string) => void; dismissEmailInboxItems: () => void }) {
  ...
  return <>
    ...
    <InboxReviewCard gptInbox={gptInbox} importNotice={importNotice} acceptInboxItem={acceptInboxItem} reviewInboxItem={reviewInboxItem} dismissInboxItem={dismissInboxItem} dismissEmailInboxItems={dismissEmailInboxItems}/>
...
function TasksPage({ tasks, taskWorkLogs, syncToken, gptInbox, importNotice, edit, remove, complete, logTime, logFocus, acceptInboxItem, reviewInboxItem, dismissInboxItem, dismissEmailInboxItems, importEmailCandidates }: { tasks: Task[]; taskWorkLogs: TaskWorkLog[]; syncToken: string; gptInbox: GptInboxItem[]; importNotice: string; edit: (t: Task) => void; remove: (id: string) => void; complete: (id: string) => void; logTime: (id: string, minutes: number) => void; logFocus: (minutes: number, details?: Pick<TaskWorkLog, 'memo' | 'startedAt' | 'endedAt'>) => void; acceptInboxItem: (item: GptInboxItem) => void; reviewInboxItem: (item: GptInboxItem) => void; dismissInboxItem: (id: string) => void; dismissEmailInboxItems: () => void; importEmailCandidates: (items: GptInboxItem[]) => void }) {
  ...
  const dailyTotal = tasks.filter(t => taskKind(t) === 'daily').length
  const dailyOpen = tasks.filter(t => taskKind(t) === 'daily' && isTaskOpenToday(t)).length
  const dailyDoneToday = Math.max(0, dailyTotal - dailyOpen)
  const temporaryOpen = tasks.filter(t => taskKind(t) === 'temporary' && isTaskOpenToday(t)).length
  const dailySummary = dailyTotal ? `毎日は${dailyDoneToday}/${dailyTotal}完了` : '毎日タスクは未登録'
  ...
  const timerStartedLabel = timerStartedAt ? new Intl.DateTimeFormat('ja-JP', { hour: '2-digit', minute: '2-digit' }).format(new Date(timerStartedAt)) : ''
  const parsedFocusMinutes = Math.round(Number(focusMinutes))
  const manualFocusMinutes = Number.isFinite(parsedFocusMinutes) ? Math.min(720, Math.max(1, parsedFocusMinutes)) : 0
  const recordManualFocus = () => {
    if (manualFocusMinutes > 0) logFocus(manualFocusMinutes, { memo: '手入力' })
  }
  const stopFocusTimer = () => {
    if (!timerStartedAt) return
    const endedAt = new Date(), minutes = Math.max(1, Math.ceil((endedAt.getTime() - timerStartedAt) / 60000))
    logFocus(minutes, { memo: '計測', startedAt: new Date(timerStartedAt).toISOString(), endedAt: endedAt.toISOString() })
    setTimerStartedAt(null)
  }
  const discardFocusTimer = () => setTimerStartedAt(null)
  ...
  const typeCards = [
    { label: 'すべて' as const, value: openCount, note: '未完了' },
    { label: '毎日' as const, value: dailyTotal ? `${dailyDoneToday}/${dailyTotal}` : 0, note: dailyTotal ? '今日完了' : '登録なし' },
    { label: '臨時' as const, value: temporaryOpen, note: '未完了' },
  ]
  return <><PageHeading eyebrow="TODO" title="やること"><>{openCount}件の未完了があります。{dailySummary}、臨時は{temporaryOpen}件。今日の集中は{todayLogged}分です。</></PageHeading>
    ...
    <InboxReviewCard gptInbox={gptInbox} importNotice={importNotice} acceptInboxItem={acceptInboxItem} reviewInboxItem={reviewInboxItem} dismissInboxItem={dismissInboxItem} dismissEmailInboxItems={dismissEmailInboxItems} eyebrow="MAIL & GPT REVIEW"/>
...
function CalendarPage({ events, syncToken, edit, remove, importEvents, updateEvents }: { events: CalendarEvent[]; syncToken: string; edit: (event: CalendarEvent) => void; remove: (id: string) => void; importEvents: (events: CalendarEvent[]) => void; updateEvents: (events: CalendarEvent[]) => void }) {
  ...
  const [calendarStatus, setCalendarStatus] = useState<CalendarConnectStatus>('idle')
  ...
  const authHeaders = useCallback(() => ({ Authorization: `Bearer ${syncToken.trim()}` }), [syncToken])
  const checkCalendarStatus = useCallback(async (silent = false) => {
    const token = syncToken.trim()
    if (!token) {
    ...
    if (!silent) setCalendarStatus('checking')
    try {
      const response = await fetch('/api/calendar-auth', { headers: authHeaders(), cache: 'no-store' })
      const payload = await response.json().catch(() => ({}))
      setCalendarNeeds(Array.isArray(payload.needs) ? payload.needs : [])
      setCalendarRedirectUri(payload.redirectUri || '')
      if (response.status === 401) {
  ...
  useEffect(() => { checkCalendarStatus(true) }, [checkCalendarStatus])
  useEffect(() => {
    const params = new URLSearchParams(location.search)
  ...
  const connectGoogleCalendar = async () => {
    const token = syncToken.trim()
    if (!token) {
      setCalendarMessage('まず設定で「同期キーを作る」を押してください。Googleカレンダー連携の本人確認にも使います。')
      return
    }
    setCalendarStatus('checking')
    setCalendarMessage('')
    try {
      const response = await fetch('/api/calendar-auth', { method: 'POST', headers: authHeaders() })
      const payload = await response.json().catch(() => ({}))
      setCalendarNeeds(Array.isArray(payload.needs) ? payload.needs : [])
      setCalendarRedirectUri(payload.redirectUri || '')
      if (response.status === 503) {
        setCalendarStatus('unconfigured')
        setCalendarMessage('Googleカレンダー連携の準備がまだです。必要な設定を確認してください。')
        return
      }
      if (response.status === 401) {
        setCalendarStatus('invalid')
      ...
      if (!response.ok || !payload.url) throw new Error(payload.error || 'calendar auth failed')
      location.href = payload.url
    } catch {
      setCalendarStatus('error')
      setCalendarMessage('Googleカレンダー接続を開始できませんでした。少し待ってからもう一度お試しください。')
    }
  }
  const syncGoogleCalendar = async () => {
    ...
    setCalendarMessage('Googleカレンダーから予定を読み込んでいます。')
    try {
      const response = await fetch('/api/calendar-events', {
        method: 'POST',
        headers: { ...authHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ daysBefore: 7, daysAfter: 120, maxResults: 80 }),
      })
      const payload = await response.json().catch(() => ({}))
      if (response.status === 409) {
        setCalendarStatus('disconnected')
      ...
      setCalendarMessage(items.length ? `${items.length}件の予定を読み込みました。既にある予定は重複しません。` : '読み込める予定は見つかりませんでした。')
    } catch {
      setCalendarStatus('error')
      setCalendarMessage('Googleカレンダーの読み込みに失敗しました。接続が切れている場合は、もう一度接続してください。')
    }
  }
  const disconnectGoogleCalendar = async () => {
    if (!confirm('Googleカレンダー連携を解除しますか？Lady Butler内のタスクや日記は消えません。')) return
    setCalendarStatus('checking')
    try {
      ...
      setCalendarStatus('error')
      setCalendarMessage('Googleカレンダー連携を解除できませんでした。少し待ってからもう一度お試しください。')
    }
  }
...
function SettingsPage({ settings, setSettings, syncToken, setSyncToken, syncStatus, syncMessage, syncNow, deviceSyncStatus, deviceSyncMessage, deviceSyncNow, pushStatus, pushMessage, enablePush, backup, restore, deleteCloud, clear }: { settings: Settings; setSettings: React.Dispatch<React.SetStateAction<Settings>>; syncToken: string; setSyncToken: React.Dispatch<React.SetStateAction<string>>; syncStatus: GptSyncStatus; syncMessage: string; syncNow: () => void; deviceSyncStatus: DeviceSyncStatus; deviceSyncMessage: string; deviceSyncNow: () => void; pushStatus: PushStatus; pushMessage: string; enablePush: () => Promise<NotificationPermission | 'unsupported'>; backup: AppBackup; restore:(data: AppBackup)=>boolean; deleteCloud:()=>Promise<boolean>; clear:()=>void }) {
  ...
  const syncLabel = syncStatus === 'connected' ? 'GPT連携 接続済み' : syncStatus === 'connecting' ? '接続を確認中' : syncStatus === 'unconfigured' ? 'ストレージ設定待ち' : syncStatus === 'invalid' ? '共通同期キーを確認' : syncStatus === 'error' ? '一時的に未接続' : 'GPT受信 待機中'
  ...
  const checkGptReadAccess = async () => {
    const token = syncToken.trim()
    if (!token) {
      setGptCheckMessage('共有データ診断には「共通の同期キー」が必要です。設定のPC・スマホ同期で作ったキーを入れてください。GPT_ACTION_TOKENではありません。')
      return
    }
    setGptChecking(true)
    setGptCheckMessage('')
    try {
      ...
      const payload = await response.json().catch(() => ({}))
      if (response.status === 401) throw new Error('共通同期キーが一致しません。PC・スマホ同期欄の「同期キーを作る」または「同期キーをコピー」で確認してください。GPT_ACTION_TOKENとは別です。')
      if (!response.ok) throw new Error(payload.error || '共有データを確認できませんでした。少し待ってから「同期を確認・開始」を押してください。')
      ...
      setGptCheckMessage(error instanceof Error ? error.message : '共有データを確認できませんでした。少し待ってから「同期を確認・開始」を押してください。')
    } finally {
  ...
  const registerGeneratedSyncToken = async (nextToken: string, previousToken: string) => {
    try {
      ...
      const payload = await response.json().catch(() => ({}))
      if (response.status === 503) return '同期キーを作りました。クラウド保存が未設定のため、先にVercelでUpstash Redisを接続してください。'
      if (response.status === 401) return '同期キーを作りましたが、クラウド側には既に別のキーがあります。今使っているキーで「同期を確認・開始」を通してから作り直してください。'
      if (!response.ok) throw new Error(payload.error || 'sync token setup failed')
      return '同期キーを作ってクラウドへ登録しました。VercelのSYNC_ACCESS_TOKEN登録は不要です。'
    } catch {
      return '同期キーを作りましたが、クラウド登録を確認できませんでした。少し待ってから「同期を確認・開始」を押してください。'
    }
  }
  const generateSyncToken = async () => {
    const previousToken = syncToken.trim()
    if (previousToken && !confirm('現在の同期キーを新しく作り直しますか？スマホ側にも新しいキーを貼り直す必要があります。')) return
    const nextToken = generateSecureSyncToken()
    setSyncToken(nextToken)
    setShowSyncKey(true)
    setTokenCopyStatus('idle')
    const registerMessage = await registerGeneratedSyncToken(nextToken, previousToken)
    try {
      await navigator.clipboard.writeText(nextToken)
      setTokenCopyStatus('copied')
      setGeneratedTokenMessage(`${registerMessage} キーもコピーしました。`)
    } catch {
      setGeneratedTokenMessage(`${registerMessage} 表示中のキーをスマホへ貼り付けてください。`)
    }
  }
  ...
  return <>
    ...
    <section className="card settings-card">
      <div className="settings-start-guide"><span>FIRST SETUP</span><h2>最初にやる3つ</h2><ol><li><b>1</b><p>PC・スマホ同期で「同期キーを作る」を押す</p></li><li><b>2</b><p>スマホにも同じ共通キーを貼る</p></li><li><b>3</b><p>Custom GPTのActions認証には、別の GPT_ACTION_TOKEN を入れる</p></li></ol></div>
      ...
      <div className="settings-section device-sync-section"><div><h2>PC・スマホ同期</h2><p>このアプリをスマホでも同じ状態で使うための設定です。GPT用のキーとは別です。</p></div><div className="gpt-link-box sync-box"><div className={`sync-state sync-${deviceSyncStatus}`} role="status"><Cloud size={16}/><strong>{deviceSyncLabel}</strong></div><div className="sync-explain-card"><strong>スマホでやること</strong><ol><li><span>1</span><p>スマホで <b>{appUrl.replace(/^https?:\/\//, '')}</b> を開く</p></li><li><span>2</span><p>左下の「設定」→「PC・スマホ同期」へ進む</p></li><li><span>3</span><p>この画面と同じ「共通の同期キー」を貼り付ける</p></li></ol></div><div className="sync-data-map"><span>同期されるもの</span><div><b>やること</b><b>予定</b><b>日記</b><b>気分</b><b>設定</b></div><small>スマホで編集した内容も、PCへ戻した時に反映されます。</small></div><Field label="共通の同期キー"><div className="sync-key-field"><input type={showSyncKey ? 'text' : 'password'} autoComplete="off" value={syncToken} onChange={e=>{ setSyncToken(e.target.value); setGeneratedTokenMessage(''); setTokenCopyStatus('idle') }} placeholder="PCとスマホで同じキー"/><button type="button" onClick={()=>setShowSyncKey(value=>!value)}>{showSyncKey ? '隠す' : '表示'}</button></div></Field><div className="sync-key-tools"><button type="button" onClick={generateSyncToken}><Sparkles size={15}/>同期キーを作る</button><small>作成したキーはクラウドへ自動登録します。Vercelの <b>SYNC_ACCESS_TOKEN</b> 手入力は不要です。</small></div>{generatedTokenMessage && <small className="sync-generated-message" role="status">{generatedTokenMessage}</small>}<div className="sync-actions"><button type="button" onClick={deviceSyncNow} disabled={!syncToken.trim() || deviceSyncStatus === 'connecting' || deviceSyncStatus === 'syncing'}><RefreshCw size={15}/>同期を確認・開始</button><button type="button" onClick={copySyncToken} disabled={!syncToken.trim()}>{tokenCopyStatus === 'copied' ? <Check size={15}/> : <Copy size={15}/>} {tokenCopyStatus === 'copied' ? 'キーをコピーしました' : '同期キーをコピー'}</button><button type="button" onClick={copyAppUrl}>{urlCopyStatus === 'copied' ? <Check size={15}/> : <Copy size={15}/>} {urlCopyStatus === 'copied' ? 'URLをコピーしました' : 'スマホ用URLをコピー'}</button></div>{tokenCopyStatus === 'error' && <small className="copy-error" role="alert">コピーできませんでした。同期キー欄を長押ししてコピーしてください。</small>}{urlCopyStatus === 'error' && <small className="copy-error" role="alert">URLをコピーできませんでした。ブラウザのアドレスをスマホへ送ってください。</small>}<div className="sync-safe-note"><strong>迷った時の見分け方</strong><p>ここに入れるのは「PCとスマホで同じキー」です。Custom GPTのAction認証キーはここには入れません。</p></div><small>{deviceSyncMessage || '初回は「同期キーを作る」を押すと、そのままクラウド登録まで行います。スマホには同じキーを貼るだけです。'}</small></div></div>
      <div className="settings-section gpt-link-section"><div><h2>GPT双方向連携</h2><p>予定の追加だけでなく、必要な時にLady Butlerの記録を読んで提案へ反映します。</p></div><div className="gpt-link-box sync-box"><div className={`sync-state sync-${syncStatus}`}><Cloud size={16}/><strong>{syncLabel}</strong></div><div className="gpt-flow-steps"><span><b>1</b>GPTで話す</span><span><b>2</b>記録を読み書き</span></div><div className="gpt-key-map"><article><span>スマホ同期</span><strong>共通の同期キー</strong><p>このアプリ画面に入れるキー。PCとスマホの保存データ確認に使います。</p></article><article><span>Custom GPT</span><strong>GPT_ACTION_TOKEN</strong><p>GPT編集画面のActions認証に入れるキー。アプリ画面には入れません。</p></article></div><div className="gpt-share-options"><strong>GPTへ共有する記録</strong><label><input type="checkbox" checked={effectiveSettings.gptShareTasks} onChange={e=>update('gptShareTasks',e.target.checked)}/><span>やること・予定</span></label><label><input type="checkbox" checked={effectiveSettings.gptShareMood} onChange={e=>update('gptShareMood',e.target.checked)}/><span>直近7日間の気分</span></label><label><input type="checkbox" checked={effectiveSettings.gptShareDiary} onChange={e=>update('gptShareDiary',e.target.checked)}/><span>直近5件の日記</span></label><small>GPTが相談に答える時だけ参照します。日記を長く引用しない設計です。</small></div><div className="gpt-behavior-note"><strong>自動整理の基準</strong><span>提出・買い物・連絡は「やること」。授業・予約・待ち合わせは「予定」。やることは締切なしでも追加し、予定の開始日時が曖昧な時だけ確認します。</span></div><div className="gpt-troubleshoot"><strong>うまく動かない時</strong><p>「GPT連携キー」と出たらGPT編集画面のActions認証を確認。「共通同期キー」と出たらこのアプリのPC・スマホ同期欄を確認してください。</p></div><div className="sync-actions"><button className="gpt-open-button" type="button" onClick={openCustomGpt}><Sparkles size={15}/>GPTを開く<ExternalLink size={12}/></button><button type="button" onClick={syncNow} disabled={!syncToken.trim() || syncStatus === 'connecting'}><RefreshCw size={15}/>受信を確認</button><button type="button" onClick={checkGptReadAccess} disabled={gptChecking}><Database size={15}/>{gptChecking ? '診断中…' : '共有データを診断'}</button></div><div className="sync-actions gpt-setup-actions"><button type="button" onClick={copyGptInstructions}>{instructionCopyStatus === 'copied' ? <Check size={15}/> : <Copy size={15}/>} {instructionCopyStatus === 'copied' ? '指示をコピーしました' : 'GPT用の指示をコピー'}</button><button type="button" onClick={copySchemaUrl}>{copyStatus === 'copied' ? <Check size={15}/> : <Copy size={15}/>} {copyStatus === 'copied' ? 'URLをコピーしました' : 'Action設定URLをコピー'}</button></div><code>{actionSchemaUrl}</code>{copyStatus === 'error' && <small className="copy-error" role="alert">コピーできませんでした。上のURLを長押ししてコピーしてください。</small>}{instructionCopyStatus === 'error' && <small className="copy-error" role="alert">GPT用の指示をコピーできませんでした。</small>}{gptCheckMessage && <small className="gpt-check-result" role="status">{gptCheckMessage}</small>}<small>{syncMessage || 'Custom GPTのActions認証にはVercelのGPT_ACTION_TOKENを登録します。ここに入れる共通同期キーとは別です。'}</small><small><a href="/privacy.html" target="_blank" rel="noreferrer">プライバシーとデータの扱い</a></small></div></div>
...
function TaskModal({ task: initial, save, close, notice: _notice }: { task: Task; save:(t:Task)=>void; close:()=>void; notice?: string[] }) {
  const [task,setTask]=useState<Task>(()=>({...initial,category:taskCategoryLabel(initial.category),actualMinutes:taskActualMinutes(initial),taskType:taskKind(initial),lastCompletedDate:initial.lastCompletedDate || '',status:taskDisplayStatus(initial)})), update=(k:keyof Task,v:string|number)=>setTask(p=>({...p,[k]:v}))
